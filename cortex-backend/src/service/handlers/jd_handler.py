...
SYSTEM_PROMPT = """You are an expert talent intelligence analyst. Extract concept entities from a job description.
...
NAMING RULES:
- Skills: 1-4 words, noun-phrase ("Python", "system design", "B2B SaaS product management"). Reject sentences.
- Trait names: 1-6 words, noun-phrase ("outcome-focused", "strong product ownership"). Reject sentences and generic words.
- Market names: industries/domains/segments ONLY ("Fintech", "B2B SaaS", "AI Infrastructure"). NEVER traits/skills/capabilities. Reject "Communication Skills", "technical knowledge", "strategic thinking".
...
- "company_traits": [{{"company": str, "name": str, "category": str, "evidence": str}}]
...
class JDHandler(BaseHandler):
    ...
    async def fetch_and_build_triplets(self, source_ref: SourceRef, org_id: str) -> list[Triplet]:
        ...
        text = await self._fetcher.fetch_job_description(source_ref.requisition_id)
        if not text:
            logger.info("no_job_description", requisition_id=source_ref.requisition_id)
            return []
    ...
    def build_triplets_from_payload(self, payload: dict, org_id: str) -> list[Triplet]:
        raise NotImplementedError("JDHandler does not support direct payload ingestion")
    ...
    def _build_triplets(self, metadata: IntakeMetadata, concepts: dict) -> list[Triplet]:
        ...
        concepts["requisition_traits"] = filter_traits(concepts.get("requisition_traits") or [], handler="jd")
        ...
        for ct in concepts.get("company_traits", []):
            ...
            triplets.append(Triplet(
                ...
                edge_attributes={"evidence": ct.get("evidence"), "source": "jd"},
            ))
