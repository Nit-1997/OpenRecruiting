...
function defaultRecent(): RecentActivity[] {
  return [
    ...
    },
    {
      id: 'a3',
      kind: 'candidate',
      label: 'Priya Natarajan advanced to panel',
      whenAgo: '3h ago',
    },
    {
      id: 'a4',
...
export const ROLE_OVERVIEWS: Record<string, RoleOverview> = {
  ...
  'des-ny': {
    ...
    recentActivity: defaultRecent(),
    owner: owner('Priya Shah', 'PS', '#E9DFF5', 'Recruiter'),
    hiringManager: owner('Priya Shah', 'PS', '#E9DFF5', 'Hiring manager'),
...
export function getRoleOverview(roleId: string): RoleOverview {
  ...
  return {
    ...
    recentActivity: defaultRecent(),
    owner: owner(role?.owner ?? 'Nitin', (role?.owner ?? 'N')[0] ?? 'N', '#EADFD4', 'Recruiter'),
    hiringManager: owner('David Woo', 'DW', '#D8EFE3', 'Hiring manager'),
