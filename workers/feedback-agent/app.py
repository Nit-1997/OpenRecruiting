"""HTTP wrapper so the feedback worker can run as a container instead of a Lambda.
...
app = FastAPI(title="feedback-agent worker")
...
class _Context:
    ...
    function_name = "feedback-agent"
    aws_request_id = "local"


@app.get("/health")
async def health() -> dict:
    return {"status": "ok", "worker": "feedback-agent"}
