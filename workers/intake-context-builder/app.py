"""HTTP wrapper so the intake-context-builder worker can run as a container instead of a Lambda.
...
app = FastAPI(title="intake-context-builder worker")
...
class _Context:
    ...
    function_name = "intake-context-builder"
    aws_request_id = "local"


@app.get("/health")
async def health() -> dict:
    return {"status": "ok", "worker": "intake-context-builder"}
