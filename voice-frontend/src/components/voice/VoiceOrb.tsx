...
export default function VoiceOrb({ state }: VoiceOrbProps) {
  ...
  const remoteAudioRef = useRef(0);
  const smoothedAudioRef = useRef(0);
  const sizeRef = useRef({ w: 0, h: 0 });
  const loCanvasRef = useRef<HTMLCanvasElement | null>(null);

  const onLocalAudio = useCallback((level: number) => {
    localAudioRef.current = level;
  }, []);
  const onRemoteAudio = useCallback((level: number) => {
    remoteAudioRef.current = level;
  }, []);

  useRTVIClientEvent(RTVIEvent.LocalAudioLevel, onLocalAudio);
  useRTVIClientEvent(RTVIEvent.RemoteAudioLevel, onRemoteAudio);
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      const rawAudioLevel =
        state === "listening" || state === "processing"
          ...
          : state === "speaking"
            ...
            : 0;
      const boostedAudio = Math.min(1, rawAudioLevel * 1.0);
      smoothedAudioRef.current +=
        (boostedAudio - smoothedAudioRef.current) * 0.08;
      ...
      const shimmerRate = Math.max(8, 18 - Math.floor(audio * 5));
      ...
      const dx = (w - dw) / 2;
      const dy = (h - dh) * 0.35;
      ctx.drawImage(loCanvas, 0, 0, N, N, dx, dy, dw, dh);
